...
void Character::initCharacterRects() {
...
Character::Character(SoundManager* sound) {
    image = al_load_bitmap("res/images/mario-small.png");
    type = Character::Small;
    initCharacterRects();
    currentFrame = stand;
    jumpSpeed = 9;
    moveSpeed = 5;
    ...
    objectType = GameObjects::Character;
    isCollidable = true;
...
void Character::unloadContect() {

}

void Character::update(ALLEGRO_EVENT event, InputManager input) {
    input.update();
...
bool Character::haveCollideWith(GameObjects* obj) {
...
void Character::collide(GameObjects* obj) {
    if (obj->objectType == GameObjects::Wall) {
        velocity.second = 0;
        box->updateWithBottom(obj->box->top);
        isOnAir = false;
        activeGravity = false;
    }
    else if (obj->objectType == GameObjects::Pipe) {
    ...
    else if (obj->objectType == GameObjects::Enemy) {
        sound->PlayPlayerDie();
    }
}
