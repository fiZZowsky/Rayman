...
void Gameplay::KeyUp(WPARAM wParam)
{
    // sets and resets key presses
    if (wParam == ALLEGRO_KEY_LEFT)
        leftPressed = false;
    if (wParam == ALLEGRO_KEY_RIGHT)
        rightPressed = false;
    if (wParam == ALLEGRO_KEY_SPACE)
        spacePressed = false;
}

void Gameplay::KeyDown(WPARAM wParam)
{
    // sets and resets key presses
    if (wParam == ALLEGRO_KEY_LEFT)
        leftPressed = true;
...
void Gameplay::Logic()
{
    ...
    if (!character->IsDead()) // all the logic below is happening only if the character is not dead
    {
        ...
        if (character->GetPosition().y > gameHeight)
        {
        ...
        Gameplay::offset = character->GetPosition().x - gameWidth / 2;
        if (Gameplay::offset < 0)
            Gameplay::offset = 0;
        if (Gameplay::offset > LEVEL_WIDTH * TILE_WIDTH - gameWidth)
            Gameplay::offset = LEVEL_WIDTH * TILE_WIDTH - gameWidth;
    }
    else
    {
